...
def init_q_table(observation_space, action_dim, discr, bool_position=None, init_q=0.0):
    ...
    for idx, (low_val, high_val) in enumerate(zip(low_values, high_values)):
        ...
        if (bool_position is not None) and (idx in bool_position):
            axis.append(
                np.linspace(low_val, high_val, 2, dtype=np.float32)
            )  # for boolean, we only have two values: 1., 0.
        else:
...
def update_q_value(
    old_state, action, new_state, gamma, reward, done, alpha, q_axis, q_table
):
...
def main(cfg):
    ...
    if cfg.save_video:
        ...
        )  # save video for testing every 5 episodes

    # init q_table with zeros
    q_axis, q_table = init_q_table(
        env.observation_space,
        env.action_space.n,
        cfg.discr,
        bool_position=cfg.bool_position,
        init_q=cfg.initial_q,
    )

    # begin training and testing
    ep_reward_deque = deque(
        [], maxlen=500
    )  # used to calculate the smooted (avg over recent 500 episodes) ep_reward
    for ep in range(cfg.train_episodes + 1):
        ...
        if (not cfg.silent) and (ep % 500 == 0):
            print(info)

    # save the q-value table and q_axis
    h.save_object({"q_table": q_table, "axis": q_axis}, work_dir / "q_table.pkl")
