...
class DQNAgent(object):
    ...
    def update(self, buffer):
        ...
        # calculate the q(s,a)
        qs = self.policy_net(batch.state).gather(
            1, batch.action.to(dtype=torch.int64)
        )  # shape: [batch_size, 1]
        ...
        torch.nn.utils.clip_grad_norm_(
            self.policy_net.parameters(), self.grad_clip_norm, error_if_nonfinite=False
        )
        self.optimizer.step()
        ########## You code ends here #########
    ...
    def get_action(self, state, epsilon=0.05):
        ...
        if sample > epsilon:
            ...
            state = torch.tensor(state).to(
                device
            )  # conver state to tensor and put it to device
