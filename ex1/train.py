...
def train(agent, env, min_update_samples=2000):
    ...
    while not done:
        ...
        agent.store_outcome(
            previous_observation, action, observation, reward, action_log_prob, done
        )
...
def test(agent, env, episodes):
    ...
    print(
        "Average test reward:",
        total_test_reward / episodes,
        "episode length:",
        total_test_len / episodes,
    )
...
def main(cfg):
    ...
    env = gym.make(
        cfg.env_name, max_episode_steps=cfg.max_episode_steps, render_mode="rgb_array"
    )
    ...
    if not cfg.testing:  # training
        for ep in range(cfg.train_episodes + 1):
            train_info = train(agent, env, min_update_samples=cfg.min_update_samples)
