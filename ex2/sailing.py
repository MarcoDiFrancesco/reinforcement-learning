...
class SailingGridworld(object):
    ...
    def __init__(
        self, rock_penalty=-2, harbour_reward=10, wind_p=0.1, value_update_iter=100
    ):
        ...
        self.value_update_iter = (
            value_update_iter  # used as a flag to save gif of value_frames
        )
        ...
        self.terminate[
            self.rocks1_x[0] : self.rocks1_x[1], self.rocks1_y[0] : self.rocks1_y[1]
        ] = 1
        self.terminate[
            self.rocks2_x[0] : self.rocks2_x[1], self.rocks2_y[0] : self.rocks2_y[1]
        ] = 1
        self.rewards[
            self.rocks1_x[0] : self.rocks1_x[1], self.rocks1_y[0] : self.rocks1_y[1]
        ] = rock_penalty
    ...
    def is_rocks(self, x, y):
        ...
        is_rocks1 = (
            self.rocks1_x[0] <= x < self.rocks1_x[1]
            and self.rocks1_y[0] <= y < self.rocks1_y[1]
        )
        is_rocks2 = (
            self.rocks2_x[0] <= x < self.rocks2_x[1]
            and self.rocks2_y[0] <= y < self.rocks2_y[1]
        )
    ...
    def is_wind(self, x, y):
        ...
        is_wind = (
            self.wind_x[0] <= x < self.wind_x[1]
            and self.wind_y[0] <= y < self.wind_y[1]
        )
        return is_wind
    ...
    def _get_next_states_wind(self, state, action):
        ...
        transitions = [
            self.Transition(
                desired_state,
                self.rewards[desired_state[0], desired_state[1]],
                self.terminate[desired_state[0], desired_state[1]],
                1 - 4 * self.wind_p,
            ),
            self.Transition(
                desired_up,
                self.rewards[desired_up[0], desired_up[1]],
                self.terminate[desired_up[0], desired_up[1]],
                self.wind_p,
            ),
            self.Transition(
                desired_down,
                self.rewards[desired_down[0], desired_down[1]],
                self.terminate[desired_down[0], desired_down[1]],
                self.wind_p,
            ),
            self.Transition(
                desired_left,
                self.rewards[desired_left[0], desired_left[1]],
                self.terminate[desired_left[0], desired_left[1]],
                self.wind_p,
            ),
            self.Transition(
                desired_right,
                self.rewards[desired_right[0], desired_right[1]],
                self.terminate[desired_right[0], desired_right[1]],
                self.wind_p,
            ),
        ]
    ...
    def _reset_figure(self):
        ...
        rocks1 = Rectangle(
            (self.rocks1_x[0] / self.w, self.rocks1_y[0] / self.h),
            (self.rocks1_x[1] - self.rocks1_x[0]) / self.w,
            (self.rocks1_y[1] - self.rocks1_y[0]) / self.h,
            facecolor="#c1c1c0",
        )
        self.ax.add_patch(rocks1)
        rocks2 = Rectangle(
            (self.rocks2_x[0] / self.w, self.rocks2_y[0] / self.h),
            (self.rocks2_x[1] - self.rocks2_x[0]) / self.w,
            (self.rocks2_y[1] - self.rocks2_y[0]) / self.h,
            facecolor="#c1c1c0",
        )
        self.ax.add_patch(rocks2)
        wind = Rectangle(
            (self.wind_x[0] / self.w, self.wind_y[0] / self.h),
            (self.wind_x[1] - self.wind_x[0]) / self.w,
            (self.wind_y[1] - self.wind_y[0]) / self.h,
            facecolor="#0F97CA",
        )
        self.ax.add_patch(wind)
        harbour = Rectangle(
            (self.harbour_x / self.w, self.harbour_y / self.h),
            1 / self.w,
            1 / self.h,
            facecolor="#7AE266",
        )
    ...
    def _get_possible_transitions_standard(self, state, action):
        ...
        transitions.append(
            self.Transition(state1, reward1, terminate1, self.wrong_action_prob)
        )
        ...
        transitions.append(
            self.Transition(state2, reward2, terminate2, self.wrong_action_prob)
        )
        ...
        transitions.append(
            self.Transition(state3, reward3, terminate3, 1 - 2 * self.wrong_action_prob)
        )
        return transitions
